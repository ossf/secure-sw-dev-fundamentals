...
For secure development you want to use a combination of both static and dynamic analysis, as we'll discuss later. This includes using modern Artificial Intelligence (AI) to enhance your verification process.
...
We strongly recommend including AI-powered security analysis to your security verification toolkit. Failing to do so is dangerous. Attackers will use AI to find vulnerabilities. If you don't use AI to find vulnerabilities, attackers are likely to find and exploit them instead.

AI is fundamentally different from other approaches. Traditional security tools are either static analysis or dynamic analysis tools, but Artificial Intelligence (AI) doesn't fit neatly into either box.  Instead, AI ends up being a powerful way to combine verification approaches.

AI-powered security tools can analyze your software using both static and dynamic approaches, often at the same time. On the static side, an AI can review the output of other static analysis tools, automatically filtering out false positives. Even more powerfully, AI can *directly* read through your source code to understand its high-level context, logic, and intent. When AI carefully reviews source code, reasoning through it with knowledge of context, it can find complex vulnerabilities and design flaws that simple rule-based matchers completely miss. On the dynamic side, AI can automatically write custom security test cases, generate smart inputs for your fuzzers, and interact with a running application to observe how it responds to unusual traffic.

In short, AI bridges the gap beyond traditional static and dynamic analysis by combining the context awareness of a human reviewer with machine-level scale. They can seamlessly pivot between reading code and running programs, so they provide an incredibly powerful way to help you catch security defects early. They can often generate proofs of vulnerability (POV), that is, inputs to prove that there *is* a vulnerability. They can also often generate patches that fix the vulnerabilities found.
...
AI systems' capability heavily depends on the context information it has available. If you provide AI with specific context, or especially useful hints, AI is typically far more effective.
