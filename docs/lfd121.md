...
*Secure Software Development Fundamentals* (LFD121 and related)
course (from the [OpenSSF Best Practices WG](https://github.com/ossf/wg-best-practices-os-developers) that are available via the
...
* **Identification & Authentication (I&A)**<br>Require users to identify themselves and prove (authenticate) their identity before doing anything that requires authorization. For example, a user might use a username or email address as their identity. A user might then use a password, passkey, or hardware token to authenticate that they really are that user. Identification and authentication are typically done by a login process.

* **Authorization**<br>Determine if the user is allowed (authorized) to do something before deciding to do it. This requires that the user be authenticated first unless *anyone* can do it. Checking for authorization is critical for implementing confidentiality and/or integrity. Watch out: the words *authentication* and *authorization* sound similar, but they are not the same thing. You may know exactly who someone is (authentication), but still not allow that person to do something (authorization).
...
There are many ways to authenticate that a user is who they say they are. The most common factors (evidence) are:

* Something you know (e.g., passwords and PINs)
* Something you have (e.g., hardware tokens)
* Something you are (e.g., fingerprints, face recognition, and iris scans)

Sometimes these factors are also used:
...
* Single-device passkeys can't be removed from the device. These passkeys are secure from many attacks, but are permanently lost if the device is lost, stolen, or damaged
* Single-ecosystem passkeys are stored in a computing ecosystem and shared among devices of that ecosystem. These are convenient but risk vendor lock-in since the passkeys often can't be extracted from their ecosystem
* Portable passkeys are stored in a third party system (such as a password manager) that permits extraction and use in multiple ecosystems

It's important to make authentication easy. An authentication scheme supports "single sign-on (SSO)" if it allows a user to log in with a single ID to any of several related, yet independent, software systems. ["Single Sign-on"](https://en.wikipedia.org/w/index.php?title=Single_sign-on&oldid=1336436965)

#### Common authorization approaches

Authorization is the process of determining if the user is allowed (authorized) to do something before deciding to do it. This determination is usually based on at least this information:

1. **Subject/Actor**: The active entity that requests access to a resource or performs an action. For example, a user, a process, or a device.
2. **Object(s)/Target(s)**: The passive entities that contain or receive information. An object could be a file, folder (directory), database, comment in a discussion board, a hardware device, and the like.
3. **Operation/Action**: The activity the subject is trying to do to the object(s), such as read, write, or execute. When discussing what operations a subject (user) is allowed to do on an object, the set of allowed operations is typically called the access rights or privileges.

You can view this as a sentence ("SUBJECT OPERATION OBJECTS").

Sometimes additional information is used (e.g., IP address, location of requester, time of day, or day of week). This additional information is sometimes used as a part of separate heuristics to detect suspicious activities and prevent their authorization.

Access control is the process by which a system grants or denies a subject's request to perform a specific operation on an object. There are many access control models, but a few are especially common:

* **Discretionary Access Control (DAC)**. DAC is the most common model. In DAC, each object has an "owner" and access is granted based on the discretion of the object's owner. Typically when an object is created its creator is the owner. This system is easy to understand, which is why it's common. However, it's vulnerable to malicious users or users being tricked, since it relies on users to maintain most of the security.
* **Mandatory Access Control (MAC)**. In MAC, subjects and objects are assigned a security label (like "Unclassified" or "Secret"), and a subject can only access an object based on the subject and object labels. This is sometimes used in governments and military environments, because it makes it harder for users to leak data. However, it can require significant administrative overhead, depending on how it's implemented.
* **Role-Based Access Control (RBAC)**. In RBAC, access is based on the subject's role (job function) within an organization, rather than their individual identity. Users are assigned a role, and permissions are assigned to a role instead of an individual. This can work well in organizations where access rights are easily assigned to roles, but where this mapping isn't as clear, RBAC doesn't work as well.
...
To create software, you need to know what you want it to do. Requirements are simply what a product or service needs to do or be. For our purposes, include in requirements anything required by law or regulation, anything required by your organizational security policy (if that applies), as well as anything important to its (potential) customers/users. If you are being paid to develop software, requirements are typically recorded somewhere.
...
* Access Control: Rules for who can access specific systems, codebases, or production environments.
* Incident Response: The procedures to follow if you discover a potential vulnerability or a security breach.
* Cryptographic Standards: The algorithms or key lengths your organization permits or forbids.

Requirements for the software you're developing might not be recorded in a single formal document. Sometimes each specific new requirement is simply accepted as an issue in an issue/bug tracker. In most software development projects, the requirements are identified over time in discussions with its users.
...
#### Tips for Identifying Security Requirements
...
The terms "trust" and "trustworthy" are fundamentally different. [NIST 800-160 (Vol. 1 Rev. 1 section 2.3)](https://csrc.nist.gov/pubs/sp/800/160/v1/r1/final) says,

"*Trust* is a belief that an entity meets certain expectations and can be relied upon.
The terms *belief* and *can* imply that trust may be granted to an entity whether the
entity is trustworthy or not." In contrast, "a *trustworthy* entity is one for which sufficient evidence exists to support its claimed trustworthiness."

In addition, "since trust is not necessarily based on a judgment of trustworthiness, the decision to trust an entity should consider the significance (i.e., consequences, effects, and impacts) of expectations not being fulfilled because of non-performance..."
...
### Protect, Detect, Respond, and being Proactive
...
In addition, be *proactive*. Don't wait for a system to be built or attacked. Security is an ongoing process, not a one-time event. Organizations should engage in *threat hunting*, that is, proactively searching for unusual behavior that may indicate the presence of malicious activity that may have slipped past initial defenses ([Baker2025](https://www.crowdstrike.com/en-us/cybersecurity-101/threat-intelligence/threat-hunting/)). Everyone should strive for the *continuous improvement* of their security practices by feeding the lessons learned from previous audits, bug reports, and even "near misses" back into your requirements, designs, implementations, and tests. Stay vigilant and adapt to the *latest trends* in cybersecurity. Attackers are constantly updating their methods. You must keep up-to-date to counter them. Proactive thinking is a vital part of effective risk management.
...
It's also important to identify your sensitive data and how it flows through the system. This includes Personally Identifiable Information (PII), system secrets (like passwords and private keys), or simply information that must be kept private. It's hard to protect data if you don't know what or where it is.

#### Recording and reviewing designs

Record your design, especially your key design decisions that impact security. There are many arguments about the best way to record designs. Some use simple text documents that describe the "big pieces" of the system. Others use simple data flow diagrams (DFDs) to help identify trust boundaries and sensitive data. Many graphical notations have been defined for recording designs. The key is to clearly record the design so all can review the decisions that matter for security, including the key third-party components being used. Then you, other people, and AI systems can all review the design to gain confidence that it is secure.  This documentation also reduces the risk that a key security decision made isn't accidentally undone by a 'quick fix' later.

#### Looking beyond your own system

Remember that you are designing a *system*, which is itself composed of other systems (e.g., external software libraries or cloud services) and must fit into a larger system of systems (e.g., a larger software application, corporate network, or a cloud environment). Recognizing that your system contains other systems, and is contained by others, helps counter attacks that exploit unintended interactions.

Look beyond your own code. Consider how these systems will interact, both inside your system and between other systems. A major source of vulnerabilities is a *conflict of assumptions* between systems. For example, your application might assume an internal service has already validated data, while that service might assume you will do so. Driving a secure architecture means identifying these interactions, and where you can, ensuring that secure design principles and trust boundaries are applied consistently across the larger environment. Use documentation and standards to help counter bad assumptions. Where you can, identify the other systems your system will interact with, and work to implement secure design principles across all systems in the larger environment.

In many organizations you'll need to partner with management to work across organizational boundaries in order to achieve this. We encourage managers less familiar with security to take our course [Security for Software Development Managers (LFD125)](https://training.linuxfoundation.org/training/security-for-software-development-managers-lfd125/).
...
Every time a program gets a request, at least from a source the program cannot completely trust (it is outside the trust boundary), the program must check the request. At the least, verify that all requests are always appropriately authenticated, authorized, and provide valid input *before* you act on that data. This principle is also called *non-bypassability*, because the point is that it must not be possible for an attacker to bypass security checks.

Some interfaces are completely public and don't require authentication or authorization. In those cases, it's appropriate to skip those checks. In practically all cases there are constraints on the input, so even for public interfaces you should check for valid input.

Identify and enforce architectural *chokepoints*, such as a clear non-bypassable API, that ensure every request is mediated. If your system is part of a larger system of systems, this mediation includes requests from those other systems. This prevents attackers from bypassing your primary security controls to access internal component interfaces directly. These chokepoints should be documented so developers of other systems will know what is required.
...
More generally, avoid mixing data where you have differing levels of trust. For example, if there are passwords or secret keys that need to be protected, store them in a *completely* separate area; don't store them inside a configuration file that does anything else.

This is a challenge for systems that use AI Large Language Models (LLMs). An LLM processes an input as a single stream. If an LLM reads user commands *and* malicious hidden commands hidden in a file, the LLM doesn't "know" which is which; to the LLM, they are all commands. LLM harnesses may attempt to make these inputs distinct, but these countermeasures are fundamentally porous and untrustworthy.
...
Many of the vulnerabilities we'll discuss later, such as Cross-Site Scripting (XSS) and SQL injection, are fundamentally failures to separate data from control.

A good example of separating data from control is using the Content Security Policy (CSP) mechanism of web browsers to state that the HTML only includes data and is *not* allowed to provide inline scripts (programs) or styles (which can also be programs). CSP can state that scripts and styles may only be downloaded from specified trusted places that you define. That way, if an attacker manages to subvert the HTML, the attacker will not be able to cause attacker-provided programs to be run. Using CSP this way greatly reduces the likelihood and impact of XSS vulnerabilities.

When designing the APIs that *your* system implements, be sure to separate data from control, and avoid requiring users to mix data with different levels of trust. Whenever possible, use 'out-of-band' signaling, where parameters with different trust levels are sent in separate, strictly-typed fields instead of using a single text stream that uses metacharacters and requires parsing.
...
The 2025-03-28 version suggests considering the following issues and information:

1. **Initial Assessment**  
   1. *Consider Necessity*. Evaluate whether the dependency can be avoided by utilizing existing components. Every new dependency increases the attack surface (a subversion of the new dependency, or its transitive dependencies, may subvert the system).  
   2. *Verify Authenticity*. Verify that the software being evaluated is the authentic version from the authorized source, not a personal fork nor an attacker-controlled fork. These techniques help counter the common "typosquatting" attack (where an attacker creates an "almost-correct" name). Check its name and the project website for the link. Verify the fork relation on GitHub/GitLab. Check if the project is affiliated with a foundation. Check its creation time and popularity.  
2. **Maintenance & Sustainability**. Unmaintained software is a risk; most software needs continuous maintenance. If it's unmaintained, it's also likely to be insecure.  
   1. *Activity Level*. Confirm significant recent activity (e.g., commits) has occurred within the previous 12 months.  
   2. *Communication*. Verify the existence of recent releases or announcements from the project maintainer(s).  
   3. *Maintainer Diversity*. Verify the presence of more than one maintainer, ideally from different organizations, to reduce single-point-of-failure risk. However, note that [many widely-used projects have one maintainer](https://opensourcesecurity.io/2025/08-oss-one-person/).  
   4. *Release Recency*. Confirm that the last release was issued within the previous 12 months.  
   5. *Version Stability*. Assess whether the version string indicates instability (e.g., begin with "0", include "alpha" or "beta", etc.).  
3. **Security Practices**  
   1. *Assessment Framework*. Consider using established assessment methodologies such as [SAFECode's guide Principles for Software Assurance Assessment (2019)](https://safecode.org/resource-managing-software-security/principles-of-software-assurance-assessment/), a multi-tiered approach for examining the software's security.  
   2. *Best Practices Certification*. Determine whether the project has earned (or is well on the way to) an [Open Source Security Foundation (OpenSSF) Best Practices badge](https://www.bestpractices.dev/).  
   3. *Dependency Management*. Verify that the package dependencies are (relatively) up to date.  
   4. *Repository Security*. Confirm that developers use code hosting security features where applicable (e.g., if they're on GitHub or GitLab, do they use branch protection).  
   5. *Security Audits*. Identify existing security audits and verify that identified vulnerabilities have been fixed. Security audits are relatively uncommon, but see OpenSSF's "[Security Reviews](https://github.com/ossf/security-reviews)".  
   6. *Security Development*. Confirm the project applies secure software development best practices as defined in the [Concise Guide for Developing More Secure Software](https://best.openssf.org/Concise-Guide-for-Developing-More-Secure-Software) or the [OpenSSF Open Source Security Baseline](https://baseline.openssf.org/).  
   7. *Security Documentation*. Verify the existence of documentation explaining why it's secure (aka an "assurance case").  
   8. *Security Response*. Assess whether the project fixes bugs (especially security bugs) in a timely manner, if they release security fixes for older releases, and if they have an LTS (Long Term Support) version.  
   9. *Security Scores*. Examine information on [https://deps.dev](https://deps.dev), including its [OpenSSF Scorecards](https://github.com/ossf/scorecard) score and any known vulnerabilities.  
   10. *Testing Practices*. Evaluate if there are automated tests included in its CI pipeline and what is its test coverage.  
   11. *Vulnerability Status*. Confirm if the current version is free of known important vulnerabilities (especially long-known ones). Organizations may want to implement the [OpenChain](https://www.openchainproject.org/) [Security Assurance Specification 1.1](https://github.com/OpenChain-Project/Security-Assurance-Specification/tree/main/Security-Assurance-Specification/1.1/en) to systemically check for known vulnerabilities on ingest and as new vulnerabilities are publicly revealed.  
4. **Usability & Security**  
   1. *Interface Design*. Verify that the interface/API is designed to be easy to use securely (e.g., if the interface implements a language, does it support parameterized queries).  
   2. *Interface Stability*. Verify that the interface/API is stable or that the project has a policy for avoiding and/or handling changes of interfaces/APIs which are breaking compatibility. An unstable API is a significant hindrance to uplifting to newer versions (e.g., to mitigate vulnerabilities).  
   3. *Secure Defaults*. Confirm that the default configuration and "simple examples" are secure (e.g., encryption turned on by default in network protocols). If not, avoid it.  
   4. *Security Guidance*. Verify if there is guidance on how to use it securely.  
...
5. **Adoption & Licensing**. Licensing frameworks, while not directly security-related, significantly impact security and sustainability postures. Projects lacking clear licensing information frequently demonstrate deficiencies in other security best practices.  
   1. *License Clarity*. Verify that every component has a license, that it's a widely-used [OSI license](https://opensource.org/licenses) if it's OSS, and that it's consistent with your intended use. Projects that won't provide clear license information are less likely to follow other good practices that lead to secure software.  
   2. *Name Verification*. Check if a similar name is more popular \- that could indicate a typosquatting attack.  
   3. *Adoption*. Assess if the software has significant use. Widely-used software is more likely to offer useful information on how to use it securely and more people will care about its security.  
   4. *Suitability*. Choose software that is a good solution for your problem. Avoid [Hype Driven Development](https://blog.daftcode.pl/hype-driven-development-3469fc2e9b22): Don't choose it merely because it's used by large companies or because it's the latest fad.  
6. **Practical Testing**  
   1. *Behavior Testing*. Try adding the dependency as a test, preferably in an isolated environment. Does it exhibit malicious behaviour, e.g., does it attempt to exfiltrate sensitive data?  
   2. *Dependency Impact*. Does it add unexpected or unnecessary indirect dependencies in production? For example, does it include production dependencies that are only required at development time or test time instead? If so, would their maintainers be willing to fix that? Every new dependency is a potential support problem or supply chain attack, so it's wise to eliminate unnecessary ones.  
7. **Code Evaluation**. Even a brief review of software (by you, someone you hire, or someone else), along with recent changes to it, can give you some insight. Here are things to consider:  
   1. *Code Completeness*. Evaluate whether there is evidence of insecure/incomplete software (e.g., many TODO statements).  
   2. *Malicious Code Check*. Analyze if there is evidence that the software is malicious. Per [*Backstabber’s Knife Collection*](https://arxiv.org/abs/2005.09535), check the installation scripts/routines for maliciousness, check for data exfiltration from \~/.ssh and environment variables, and look for encoded/obfuscated values that are executed. Examine the most recent commits for suspicious code (an attacker may have added them recently).  
   3. *Sandbox Testing*. Consider running the software in a sandbox to attempt to trigger and detect malicious code.  
   4. *Security Implementations*. When reviewing its source code, is there evidence in the code that the developers were trying to develop secure software (such as rigorous input validation of untrusted input and the use of parameterized statements)?  
   5. *Security Reviews*. See the [OpenSSF's list of security reviews](https://github.com/ossf/security-reviews/blob/main/Overview.md#readme).  
   6. *Static Analysis*. What are the "top" problems reported by static analysis tools?  
   7. *Test Validation*. Consider running all defined test cases to ensure the software passes them.
...
That said, in many systems, attackers will be able to attempt some requests.

##### Public-key authentication

In a public-key authentication system, one side has a secret key to prove who they are. The other side has a public key that doesn't need to be secret, but instead can be used to cryptographically determine if the other side knows the secret key. Its security depends on keeping the secret key a secret.

##### Authentication protocols

Identification and authentication are actually a two-way street, as users need to identify and authenticate:

1. the *external system* (typically a server) that they are communicating with
2. *themselves* to that external system

Many protocols build in authentication mechanisms.  Some common protocols that include authentication include:

* Transport Layer Security (TLS) is a widely-used cryptographic protocol to provide security over a network between two parties. TLS replaced the older protocol Secure Sockets Layer (SSL), but sometimes the term SSL is used for both. When you use **https&#58;//** in a web browser or server today, you are normally using TLS. TLS builds in authentication of the *server* to the *user* through certificate authorities, and this capability is widely used. TLS also supports authenticating *users* to *servers* using "client-side certificates" stored on the client; this capability is used, but user authentication is usually handled in other ways.
* SSH (Secure Shell) is a cryptographic network protocol to perform actions securely, such as remote login and file transfers. When you connect to a new server, the SSH client displays the server's public key fingerprint and asks you to confirm if it is legitimate. If approved, the public key is stored by the user as a known host for later verification (a process called "Trust on First Use"). SSH implementations typically support a variety of mechanisms for authenticating the user to the server, including passwords, public-key authentication, and so on.

In a federated identity system, a server (such as a website) delegates authentication of a user to a third party. As a result, the server doesn't need to manage authentication itself, and users can use the same third party to authenticate to many servers. A federated identity system involves two parties other than the user:
...
The **Credential Management API** is a client-side JavaScript API that provides a standardized way for websites to interact with a user’s credentials. It provides standard mechanisms for creating, storing locally (or interacting with the system that does), and retrieving credentials. User interface (UI) actions are triggered as a side effect of these actions, providing a clear standard UI. A credential is anything which enables a system to make an authentication decision, such as a password. [Credential Management API](https://developer.mozilla.org/en-US/docs/Web/API/Credential_Management_API)

The **Web Authentication API**, aka **WebAuthn**, is an extension of the Credential Management API that supports public key cryptography. This works with an "Authenticator" that stores and manages keys. On the web, passkeys are implemented using WebAuthn. WebAuthn has several strengths:

* *Protection against phishing*: An attacker can't create a fake login website, trick the user into logging in, and later log in to the real site. The signature is bound to the website's origin, and the server provides a unique 'challenge' for every request, ensuring that intercepted login data cannot be reused.
* *Reduced impact of data breaches*: Passwords must be stored carefully. In contrast, an attacker who gets access to the public key information cannot authenticate as that user (that requires the private key, which the server never has)
* *Invulnerable to password attacks*: Some users reuse passwords; attackers who discover that reused password can use it to attack other sites. In addition, passwords are vulnerable to brute force attacks. Private keys that are specific to a particular site don't have these weaknesses. [Web Authentication API](https://developer.mozilla.org/en-US/docs/Web/API/Web_Authentication_API)

#### Authorization

A reminder: authorization is the process of determining if the user is allowed (authorized) to do something before deciding to do it.

You need to *check* whether or not a request is *authorized* in *absolutely every case* if it might not be. That is to say, ensure that authorization checks are non-bypassable. Most tools are not good at determining if every request is checked for authorization, so you typically need to depend primarily on human review. If humans can easily see that the correct authorization check is made for every request, it takes much less time to review *and* it is more likely to be correct. In practice, that often means that programs should check for authorization as soon as you reasonably can do so. Exactly what that means depends on your system, e.g., in a model-view-controller architecture, you could put authorization checks on each controller entry and/or each model entry. What matters is that you do it consistently and that it is easy for others to verify that it cannot be bypassed. Similarly, the data needs to be stored so that only authorized requests can succeed.
...
Sometimes you want to separate the program that does authentication, and the program that determines what is authorized, from the program that needs to be authorized. In that case, you need an authorization framework.
...
* **Client:** The application requesting access to the data (e.g., a printing service).
* **Resource Owner:** The user who owns the data. The client sends an authorization request (including a PKCE challenge) to the resource owner via the Authorization Server, and receives an authorization code (a short-lived intermediary).
* **Authorization Server:** The server that authenticates the owner and issues tokens after obtaining their consent. The client sends the authorization code and the PKCE verifier to the authorization server. The authorization server verifies that the verifier matches the original challenge, and issues an access token back to the client.
* **Resource Server:** The server hosting protected data. The client sends the access token to the resource server, and receives the protected data.

Here are some advantages of OAuth over sharing a password:

* **Credential Isolation:** The Client application never sees or stores the user's password.
* **Scoped Access:** Unlike a password, which provides total account control, an OAuth token can be restricted to "read-only" or to specific folders.
* **Revocation:** A user can revoke a specific application's access at any time through the Authorization Server without needing to change their password or affecting other applications.

For more see [OAuth 2.0 specification, RFC 6749](https://datatracker.ietf.org/doc/html/rfc6749).
...
Historically system developers wrote "hardening guides" to improve security. Today hardening guides are considered poor practice. Systems should instead be secure by default, with an optional "loosening guide" to explain how to remove specific security features where necessary (along with an explanation of their risks).
...
Document your configuration options to explain what they do. If any have security ramifications, explain what those ramifications are. If a configuration option mitigates an attack, clearly explain what it is. Users may choose to disable some protection, but they should receive a clear explanation of its impact.

Document exactly where secrets are stored and the process for rotating secrets. Where practical, store secrets separately from other configuration information.
...
A component that supports metacharacters can be dangerous to use, because their interface fundamentally violates the design principle of separating *data* from *control* (programs). In practice, you often need to use them. To counter this problem, you need to ensure that *when* you send data to some program (or output), you send it in a secure way. That may involve:
...
For secure development, you should use a combination of both static and dynamic analysis, as we'll discuss later. This includes using Artificial Intelligence (AI) to enhance your verification process.
...
We strongly recommend including AI-powered security analysis in your security verification toolkit. Attackers are actively using AI to find and exploit flaws. Developers who fail to use AI to find and fix vulnerabilities create an unnecessary asymmetric disadvantage, essentially leaving the software open to automated attacks.

AI is fundamentally different from other approaches. Many traditional security tools are either static analysis or dynamic analysis tools, while Artificial Intelligence (AI) can be a powerful way to combine verification approaches.

AI-powered security tools can analyze your software using both static and dynamic approaches, often at the same time. On the static side, an AI can review the output of other static analysis tools, automatically filtering out false positives. Even more powerfully, AI can *directly* read through your source code to understand its high-level context, logic, and intent. When you use AI to review source code in detail, reasoning through it with knowledge of context, it can find complex vulnerabilities and design flaws that simple rule-based matchers completely miss. On the dynamic side, AI can automatically write custom security test cases, generate smart inputs for your fuzzers, and interact with a running application to observe how it responds to unusual traffic.

In short, AI bridges the gap between traditional static and dynamic analysis by combining the context awareness of a human reviewer with machine-level scale. It can seamlessly pivot between reading code and running programs, providing an incredibly powerful way to help you catch security defects early. These tools can often generate proofs of vulnerability (POV), that is, inputs that prove a vulnerability exists. These tools can also often provide patches to fix the discovered flaws.

Of course, no tool is a silver bullet. AI tools can still generate false positives or completely miss subtle vulnerabilities. They can even generate completely incorrect claims. Thus, they don't replace human thought or your existing tools. If the results matter, have a knowledgeable human review AI results.

Some AI models are better than others. However, if you do not have access to the world's best models, or can't afford them, then use what you *can* use. Don't wait for some "perfect tool"; start by using what you have available. Where practical, use multiple AI models to review your software.

An AI system's capability heavily depends on the context information it has available. If you provide the system with specific context, or especially useful hints, it is typically far more effective.

Unsurprisingly, AI is far more effective when it has useful documentation on the program it's analyzing. At least, a description of the security requirements and context is extremely important. Inline code documentation can significantly improve its ability to statically analyze code. AI can also help *generate* that documentation. If you do, have it improve the documentation "bottom up" so that the AI can use the documentation of lower-level components when it's writing the documentation of higher-level components.
...
An important first step is to decide on the scope of your threat model. While it is tempting to focus only on the specific system you are developing, in reality, you may need to include components or services outside your own system. Doing this can help you identify and mitigate threats that try to exploit the seams between systems. Adopting this "system of systems" viewpoint is important for defining and driving a secure architecture across a larger environment. By broadening your scope to include external interactions, you can identify where security assumptions might conflict, and you can work to ensure that design principles are applied consistently across all systems involved. A broader scope requires more information from others and takes more time, though, so you must determine the appropriate scope for your needs.

In many organizations you'll need to partner with management to get the data you need, weigh threats, weigh mitigations, and enlist others as needed to help you. This means managers will need to understand the issues at a high level to make appropriate decisions. We encourage managers less familiar with security to take our course [Security for Software Development Managers (LFD125)](https://training.linuxfoundation.org/training/security-for-software-development-managers-lfd125/).
...
3. The system design (possibly including the larger system of systems it's part of)
...
Here are a few threat modeling approaches [[Shevchenko2018](https://www.sei.cmu.edu/blog/threat-modeling-12-available-methods/)]:

* Process for Attack Simulation and Threat Analysis (PASTA): This is a attack-centric and risk-centric approach with 7 stages, each with multiple activities. PASTA aims to bring business objectives and technical requirements together, working to bring in key decision makers (not only technical personnel). It can view attacks strategically, but its attacker-centric analysis and focus on asset-centric output requires significant knowledge and expertise. It also requires active participation from business decision makers.
* Trike: Trike is a security audit framework that uses threat modeling as a technique. In Trike the analyst defines a system and builds a requirement model, resulting in an actor-asset-action matrix where the columns represent assets and the rows represent actors. The analyst then determines, for each cell, the permission (allowed, disallowed, or allowed with rules) for each of four actions (creating, reading, updating, deleting). The analyst then identifies threats ("elevation of privilege" or "denial of service"). Note the importance of identifying assets in this approach.
* Operationally Critical Threat, Asset, and Vulnerability Evaluation (OCTAVE): This is a cybersecurity risk-based strategic assessment and planning method. It focuses on organizational risks, not technological risks.

There is no one "perfect" threat modeling approach. However, many developers have found it easy to get started with an approach called STRIDE. So in the next unit we will focus on STRIDE.
...
STRIDE is one of the oldest, most well-known, and simplest forms of threat modeling ([*Threat Modeling: Uncover Security Design Flaws Using the STRIDE Approach*](https://web.archive.org/web/20070303103639/http://msdn.microsoft.com/msdnmag/issues/06/11/ThreatModeling/default.aspx), by Shawn Hernan, Scott Lambert, Tomasz Ostwald, and Adam Shostack, 2006). A widely-read book on threat modeling, that includes a discussion of STRIDE, is "Threat Modeling: Designing for Security" by Adam Shostack.

You can implement STRIDE with basic tools such as a drawing tool, word processor, and/or spreadsheet. There are also tools that directly support STRIDE (and/or other threat modeling approaches), for example:
...
#### STRIDE: Threat identification exercise

\>\> Apply this STRIDE threat identification scenario:<<

STRIDE isn't complicated, it's more like a structured brainstorming exercise.
However, it's much easier to understand STRIDE once you've applied key parts.
So we'll go through a simple threat identification exercise to help
you understand STRIDE.

In this scenario, we have a web application system which we will model
as three nodes:

* **Web Server / Infrastructure:** the network-facing server, hosting
  infrastructure, operating system, and web server hosting the web application
* **Web Application:** the server-side application code,
  frameworks, and session management
* **Database:** the backend data store

Apply STRIDE threat identification by
identifying a threat for each node, for each letter of the STRIDE process.

Below we list 18 threat descriptions (T1 through T18).
Each threat best applies to **exactly one** of the
three nodes and STRIDE letter.
Fill in the threat number in the cell for the node
and STRIDE letter it primarily affects (omit the "T" in your answers):

| Node | S | T | R | I | D | E |
|------|---|---|---|---|---|---|
| Web Server / Infrastructure | __ | __ | __ | __ | __ | __ |
| Web Application | __ | __ | __ | __ | __ | __ |
| Database | __ | __ | __ | __ | __ | __ |

Recall that STRIDE stands for:
...
T1. An attacker uses stolen credentials to open a direct TCP connection to the production database server and authenticate as the application's own service account, bypassing the application tier entirely.

T2. Unhandled exceptions in the server-side application cause a full stack trace (including internal file paths, library versions, and possibly private keys) to appear in the HTTP response body sent to the user.

T3. A SYN flood exhausts all available TCP connection slots on the network-facing server, preventing legitimate connections from being established.

T4. An attacker with write access to the backup storage server replaces the nightly database backup with a crafted file in which several financial records have been altered; when the backup is restored after a subsequent failure, the falsified data silently overwrites the correct production records.

T5. A regular user discovers that replacing `/user/profile` with `/admin/users` in the URL gives full access to the administration panel; the application performs no authorization check on the new path.

T6. The web server is deployed with all request logging disabled; when a security incident is investigated, there is no record of which IP addresses connected, which URLs were requested, or at what times, making it impossible to establish what an attacker accessed or when.

T7. A nightly database backup is written as a plaintext file and placed on a shared network drive that is readable by all employees.

T8. An attacker crafts a forged 'remember-me' cookie that causes the application to treat the attacker as a logged-in user without knowing that user's password.

T9. An attacker with shell access to the server modifies the web server's configuration file to add a reverse-proxy directive that silently forwards all requests for `/api/payment` to an attacker-controlled host before they reach the application.

T10. The database audit-log feature is disabled in production, so no record exists of which account executed a destructive `DELETE FROM orders` statement.

T11. An attacker submits a POST request whose body contains a deeply nested JSON object; the web server forwards the raw bytes to the application process without parsing them, as expected, but the application framework's own JSON deserialization library triggers exponential object allocation as it recurses into each level, exhausting the application process's heap and killing the worker thread.

T12. An attacker compromises the DNS resolver the web server itself uses for outbound API calls; the server's requests to a payment processor and a fraud-detection service are silently redirected to attacker-controlled hosts that return plausible but falsified responses.

T13. The application's database service account was provisioned with full privileges and the ability to grant those same privileges to other accounts; a compromised attacker using that account creates additional database accounts with full administrative control over all data and database objects.

T14. The web application stores the item price in a hidden HTML form field and trusts the client-submitted value without re-checking it against the server-side price; an attacker modifies this field and completes a purchase at an arbitrary price.

T15. The web server is configured to serve all files under the document root; because the `.git` directory was left inside the document root during deployment, any requester can download the complete Git repository history, including full source code, past configuration files, and any credentials or private keys ever committed.

T16. The web application logs that a password-reset email was sent but does not record which authenticated administrator account triggered the action.

T17. Because the web server process runs as `root`, any code-execution vulnerability in the server immediately grants the attacker full operating-system privileges.

T18. An attacker sends a flood of TCP connections directly to the database port; each connection begins the authentication handshake but never completes it, exhausting the database engine's finite pool of connection-handler slots and preventing the application from acquiring any new database connections.
...
| Node | S | T | R | I | D | E |
|------|---|---|---|---|---|---|
| Web Server / Infrastructure | T12 | T9 | T6 | T15 | T3 | T17 |
| Web Application | T8 | T14 | T16 | T2 | T11 | T5 |
| Database | T1 | T4 | T10 | T7 | T18 | T13 |

**Detailed Explanations**

**T1. Database / Spoofing (S)**

> "An attacker uses stolen credentials to open a direct TCP connection to the production database server and authenticate as the application's own service account, bypassing the application tier entirely."

The threat is at the Database node (not Web Application) because the attacker connects directly to the database server, never passing through application code; it is Spoofing (not Tampering or Elevation of Privilege) because the harm is impersonating the service account's identity at the database authentication layer, not modifying data or acquiring new database capabilities.

**T2. Web Application / Information Disclosure (I)**

> "Unhandled exceptions in the server-side application cause a full stack trace (including internal file paths, library versions, and possibly private keys) to appear in the HTTP response body sent to the user."

The threat is at the Web Application node (not Web Server) because the disclosure is produced by the application's own error-handling code, not by a server configuration header; it is Information Disclosure (not Tampering or Repudiation) because internal implementation details are exposed to unauthorized parties without any stored data being modified.

**T3. Web Server / Infrastructure / Denial of Service (D)**

> "A SYN flood exhausts all available TCP connection slots on the network-facing server, preventing legitimate connections from being established."

The threat is at the Web Server/Infrastructure node (not Web Application or Database) because the TCP connection table is a kernel resource managed by the OS network stack; it is Denial of Service (not Tampering) because no data is modified: only the server's capacity to accept new connections is eliminated.

**T4. Database / Tampering (T)**

> "An attacker with write access to the backup storage server replaces the nightly database backup with a crafted file in which several financial records have been altered; when the backup is restored after a subsequent failure, the falsified data silently overwrites the correct production records."

The threat is at the Database node (not Web Application or Web Server) because the asset being tampered with is the database's own backup file, and the harm is realized when that file is restored directly into the database engine, with no application code involved; it is Tampering (not Information Disclosure or Spoofing) because stored data values are persistently falsified rather than merely read or identity being claimed.

**T5. Web Application / Elevation of Privilege (E)**

> "A regular user discovers that replacing `/user/profile` with `/admin/users` in the URL gives full access to the administration panel; the application performs no authorization check on the new path."

The threat is at the Web Application node (not Web Server) because the missing authorization check is a flaw in the application's routing and access-control logic, not in the server's OS or network configuration; it is Elevation of Privilege (not Spoofing) because the attacker retains their actual, unprivileged identity while gaining access to capabilities reserved for administrators.

**T6. Web Server / Infrastructure / Repudiation (R)**

> "The web server is deployed with all request logging disabled; when a security incident is investigated, there is no record of which IP addresses connected, which URLs were requested, or at what times, making it impossible to establish what an attacker accessed or when."

The threat is at the Web Server/Infrastructure node (not Web Application) because request logging is a function of the web server process and its configuration file, not of application code; it is Repudiation (not Information Disclosure) because the harm is the inability to attribute network-layer actions to specific requesters, not the exposure of confidential data to unauthorized parties.

**T7. Database / Information Disclosure (I)**

> "A nightly database backup is written as a plaintext file and placed on a shared network drive that is readable by all employees."

The threat is at the Database node (not Web Server) because the exposed asset is the database's full data set, not server configuration or application source code; it is Information Disclosure (not Tampering) because unauthorized parties can read the data without modifying it.

**T8. Web Application / Spoofing (S)**

> "An attacker crafts a forged 'remember-me' cookie that causes the application to treat the attacker as a logged-in user without knowing that user's password."

The threat is at the Web Application node (not Web Server) because the cookie is validated by the application's own session-management code, not by the web server or OS; it is Spoofing (not Elevation of Privilege) because the attacker is impersonating a specific authenticated user identity rather than acquiring new capabilities beyond those already held by that user.

**T9. Web Server / Infrastructure / Tampering (T)**

> "An attacker with shell access to the server modifies the web server's configuration file to add a reverse-proxy directive that silently forwards all requests for `/api/payment` to an attacker-controlled host before they reach the application."

The threat is at the Web Server/Infrastructure node (not Web Application) because the web server's configuration file is owned and read exclusively by the web server process itself; the application code is completely untouched and unaware of the redirection; it is Tampering (not Spoofing) because a persistent artifact (the configuration file) is altered, rather than an identity being falsely claimed.

**T10. Database / Repudiation (R)**

> "The database audit-log feature is disabled in production, so no record exists of which account executed a destructive `DELETE FROM orders` statement."

The threat is at the Database node (not Web Application) because audit logging at the database engine level is a database configuration feature that records which principal ran which SQL statement, completely independent of application logging; it is Repudiation (not Information Disclosure) because the harm is the inability to attribute a destructive action to a specific account, not the exposure of confidential data to an unauthorized party.

**T11. Web Application / Denial of Service (D)**

> "An attacker submits a POST request whose body contains a deeply nested JSON object; the web server forwards the raw bytes to the application process without parsing them, as expected, but the application framework's own JSON deserialization library triggers exponential object allocation as it recurses into each level, exhausting the application process's heap and killing the worker thread."

The threat is at the Web Application node (not Web Server) because the web server plays no role in parsing the request body: it forwards the raw bytes to the application process, and the crash occurs inside the application framework's own deserialization library; it is Denial of Service (not Tampering) because no data is modified: only the availability of the application worker is eliminated.

**T12. Web Server / Infrastructure / Spoofing (S)**

> "An attacker compromises the DNS resolver the web server itself uses for outbound API calls; the server's requests to a payment processor and a fraud-detection service are silently redirected to attacker-controlled hosts that return plausible but falsified responses."

The threat is at the Web Server/Infrastructure node (not Web Application) because the poisoned resolver affects the server's own outbound DNS lookups for infrastructure-level service connections; it is Spoofing (not Tampering) because attacker-controlled hosts impersonate legitimate third-party services to deceive the server, rather than modifying data already stored by the server.

**T13. Database / Elevation of Privilege (E)**

> "The application's database service account was provisioned with full privileges and the ability to grant those same privileges to other accounts; a compromised attacker using that account creates additional database accounts with full administrative control over all data and database objects."

The threat is at the Database node (not Web Application) because the privilege escalation occurs entirely within the database engine's own permission system; it is Elevation of Privilege (not Spoofing) because the attacker genuinely acquires new capabilities (the right to create fully privileged database accounts) rather than merely impersonating an existing higher-privileged account.

**T14. Web Application / Tampering (T)**

> "The web application stores the item price in a hidden HTML form field and trusts the client-submitted value without re-checking it against the server-side price; an attacker modifies this field and completes a purchase at an arbitrary price."

The threat is at the Web Application node (not Web Server) because the flaw is in the server-side business logic that accepts the tampered value without validation; it is Tampering (not Spoofing or Elevation of Privilege) because the attacker modifies a specific data value (the price) that the application then acts on as if legitimate.

**T15. Web Server / Infrastructure / Information Disclosure (I)**

> "The web server is configured to serve all files under the document root; because the `.git` directory was left inside the document root during deployment, any requester can download the complete Git repository history, including full source code, past configuration files, and any credentials or private keys ever committed."

The threat is at the Web Server/Infrastructure node (not Web Application) because the exposure is caused by the web server's static-file serving configuration, not by any application logic; the application code is never invoked for these requests; it is Information Disclosure (not Tampering) because sensitive assets are read by unauthorized parties without any data being modified.

**T16. Web Application / Repudiation (R)**

> "The web application logs that a password-reset email was sent but does not record which authenticated administrator account triggered the action."

The threat is at the Web Application node (not Web Server or Database) because the logging gap is in the application's own audit-logging code that records administrative actions; it is Repudiation (not Information Disclosure) because the harm is the inability to attribute a sensitive action to a specific actor, not the exposure of confidential data to an unauthorized party.

**T17. Web Server / Infrastructure / Elevation of Privilege (E)**

> "Because the web server process runs as `root`, any code-execution vulnerability in the server immediately grants the attacker full operating-system privileges."

The threat is at the Web Server/Infrastructure node (not Web Application) because the misconfigured process ownership is an OS-level infrastructure setting, not an application configuration option; it is Elevation of Privilege (not Tampering) because exploiting any server vulnerability yields OS-level capabilities that far exceed those appropriate for the web server process.

**T18. Database / Denial of Service (D)**

> "An attacker sends a flood of TCP connections directly to the database port; each connection begins the authentication handshake but never completes it, exhausting the database engine's finite pool of connection-handler slots and preventing the application from acquiring any new database connections."

The threat is at the Database node (not Web Application or Web Server) because the resource exhausted is the database engine's own connection-handler pool; because the attack connects directly to the database port and bypasses the application tier entirely, the application has no opportunity to intercept or prevent it; it is Denial of Service (not Tampering) because no data is modified: only the database's ability to accept new connections is eliminated.

</details>

#### STRIDE: Threat mitigation exercise

\>\> Apply this STRIDE threat mitigation scenario:<<

In this scenario,
a server-side web application accepts user input,
queries a backend database, and
communicates with web clients (browsers) over a network.

Each of the four threat descriptions below (T1-T4) involves a different
potential vulnerability. Each of the four mitigation descriptions
(M4-M1) directly prevents exactly one of those threats.

Match each threat to the mitigation that most directly prevents it.
Each mitigation has exactly one correct match.

**T1.** An attacker submits a carefully constructed string in the username
field of the application's login form; the string terminates the intended
database query early and appends a condition that is always true, causing
the database to grant the attacker access as the first account in the table
without knowing any valid password.

Answer: __

**T2.** An attacker submits a comment through the application's public
comment form containing a script element as part of the text; the
application stores the comment in the database without modification. Every
subsequent user who views the page containing that comment causes their
browser to execute the attacker's script, which silently reads the user's
session token and transmits it to a server the attacker controls.

Answer: __

**T3.** An attacker positioned on the same network segment as the user
captures packets traveling between the user's machine and the web server;
because the connection is unencrypted, the attacker reads the user's
session token directly from the captured traffic and uses it to take over
the authenticated session.

Answer: __

**T4.** An attacker runs an automated script that submits thousands of
login requests to the application in rapid succession, each one trying a
different password from a list of commonly used passwords, until the
...
**M1.** The application enforces a limit on the number of consecutive failed
login attempts allowed within a time window; when the limit is exceeded,
further attempts are temporarily blocked or deliberately delayed, making
automated trial of large password lists impractical.

**M2.** The server requires all client connections to use TLS (Transport
Layer Security), so that all data exchanged between the client and server is
cryptographically encrypted; a network observer can see that a connection
exists but cannot read its contents.

**M3.** Before inserting any user-supplied text into an HTML page, the
application's framework encodes characters that have special meaning in HTML (for
example, replacing `<` with `&lt;` and `>` with `&gt;`), so that a browser
renders the text as literal content and never interprets it as markup or
executable script.

**M4.** The application constructs all database queries using parameterized
statements, so that user-supplied input is always passed to the database engine as a literal
data value and is never interpreted as part of the query syntax.

---

**Answers**

*(Do not read until you have completed the matching exercise.)*
...
**T1 → M4 (SQL Injection prevented by Parameterized Statements)**

> Threat T1: An attacker submits a carefully constructed string in the username field of the application's login form; the string terminates the intended database query early and appends a condition that is always true, causing the database to grant the attacker access as the first account in the table without knowing any valid password.

> Threat Mitigation M4: The application constructs all database queries using parameterized statements, so that user-supplied input is always passed to the database engine as a literal data value and is never interpreted as part of the query syntax.

M4 is the correct match because the attack succeeds only when the application concatenates user input directly into a query, allowing that input to alter the query's structure. Parameterized statements eliminate this possibility: user input is always treated as a literal value, never as syntax, regardless of what characters it contains. The other mitigations (TLS, output encoding, rate limiting) have no effect on whether a SQL injection payload succeeds.

**T2 → M3 (Stored Cross-Site Scripting prevented by Output Encoding)**

> Threat T2: An attacker submits a comment through the application's public comment form containing a script element as part of the text; the application stores the comment in the database without modification. Every subsequent user who views the page containing that comment causes their browser to execute the attacker's script, which silently reads the user's session token and transmits it to a server the attacker controls.

> Threat Mitigation M3: Before inserting any user-supplied text into an HTML page, the application's framework encodes characters that have special meaning in HTML (for example, replacing `<` with `&lt;` and `>` with `&gt;`), so that a browser renders the text as literal content and never interprets it as markup or executable script.

M3 is the correct match because the attack succeeds only when the browser interprets the attacker's stored text as executable script; HTML output encoding ensures the angle brackets and other markup characters are treated as plain text, so the script element is displayed as characters on screen rather than executed. Parameterized statements, TLS, and rate limiting do not affect how the application encodes text before writing it into the HTML response.

**T3 → M2 (Session Eavesdropping prevented by TLS)**

> Threat T3: An attacker positioned on the same network segment as the user captures packets traveling between the user's machine and the web server; because the connection is unencrypted, the attacker reads the user's session token directly from the captured traffic and uses it to take over the authenticated session.

> Threat Mitigation M2: The server requires all client connections to use TLS (Transport Layer Security), so that all data exchanged between the client and server is cryptographically encrypted; a network observer can see that a connection exists but cannot read its contents.

M2 is the correct match because the attack requires reading the contents of network packets, which TLS prevents by encrypting all data in transit. CSRF tokens, parameterized statements, and rate limiting all leave network traffic readable by an observer and therefore do not prevent session hijacking via eavesdropping.

**T4 → M1 (Brute-Force Login Attack prevented by Rate Limiting)**

> Threat T4: An attacker runs an automated script that submits thousands of login requests to the application in rapid succession, each one trying a different password from a list of commonly used passwords, until the application accepts one.

> Threat Mitigation M1: The application enforces a limit on the number of consecutive failed login attempts allowed within a time window; when the limit is exceeded, further attempts are temporarily blocked or deliberately delayed, making automated trial of large password lists impractical.

M1 is the correct match because the attack depends entirely on the ability to make a large number of login attempts quickly without consequence; capping the attempt rate or blocking the account after repeated failures makes the attack impractical within any useful time window. TLS, parameterized statements, and CSRF tokens place no restriction on how many login requests an attacker can submit per second.
...
That said, in *some* systems cryptography is a vitally important part of making a system secure.

Cryptography is often used to protect sensitive data’s confidentiality through *encryption*. Cryptography can also, with certain limits, verify data integrity and source (the latter through "digital signing"). Cryptography can be applied to data:

1. *at rest*: storing the information using cryptography
2. *in transit* (aka in motion): transmitting the information using cryptography
3. *in use*: using the data while it stays encrypted and/or is in a cryptographically-protected subsystem

The first two are more common, but there are cases where the third case is useful.

Failing to use cryptography at rest or in transit in cases where it should be used is a security vulnerability. Information that is not encrypted is often called “cleartext” or “plaintext”.  In many networks (including the Internet and its subset the world wide web), as well as many storage systems (such as backups), plaintext can be intercepted and modified by unauthorized parties.
...
Today it's generally best practice to use a mode that provides Authenticated Encryption with Associated Data (AEAD). Many modes only provide confidentiality; an attacker might be able to corrupt the encrypted text without being detected. AEAD modes provide both confidentiality *and* integrity, because they include a "tag" to prove that the data has not been tampered with after it was encrypted.

The most common AEAD mode is today is Galois/Counter Mode (GCM). GCM is fast, parallelizable, and adds an authentication code so it can easily detect if the wrong key is used. GCM is so named because includes both a *counter* to enable parallelizable encryption and *Galois* mode to provide an integrity check. It is a good mode to use. The combination of AES in GCM mode is widespread. There are other good modes as well; the important thing is to choose a mode wisely, and in particular, to *never* use ECB mode in production systems.
...
Which encryption mode is fast, parallelizable, and includes an authentication code to
...
(Yolan Romailler,
[*“Modulo Biases and how to avoid them!”*](https://romailler.ch/2020/07/28/crypto-modulo_bias_guide/))
...
Transport Layer Security (TLS) is a widely-used cryptographic protocol to provide security over a network between two parties. It provides privacy and integrity between those parties. An older and insecure version of this protocol was named Secure Sockets Layer (SSL), and sometimes the terms are used interchangeably. When you use **https&#58;//** in a web browser or server today, you are normally using TLS (in rare cases, you might be using its insecure predecessor, SSL). TLS is also used in other applications, for example, to protect exchanges of email between different Mail Transport Agents (MTAs).
...
"Confidential computing" is the approach of performing computations in a specialized hardware-based trusted execution environment (TEE).  A TEE, also called an "enclave", can be implemented in a computer's processing components such as a central processing unit (CPU) or graphics processing unit (GPU).  In confidential computing, confidential data is released to the TEE only once it is assessed to be trustworthy.  The goal of a TEE is to ensure that even if the operating system or cloud administrator is compromised, the data and code inside the enclave remain private and untampered.  A TEE must provide isolation (preventing unauthorized access) and remote attestation (proving that the environment is genuine and running the expected code). A TEE can be implemented in many ways, and its implementation typically depends in part on cryptography.

TEE implementations have been repeatedly broken when attackers have access to the physical hardware. Therefore, avoid depending on a TEE if the attacker can control the computing hardware. Instead, consider a TEE as a potential mechanism to strengthen least privilege when you trust those who control the hardware.

#### Homomorphic Encryption

Homomorphic encryption is an approach where computations can be performed directly on encrypted data (ciphertext) without ever decrypting it. This provides far stronger confidentiality, since the data is never decrypted during those computations. Instead, users can retrieve the computed results and decrypt them later using encryption keys they keep. However, these computations take orders of magnitude more calculations.

Fully homomorphic encryption (FHE) supports the evaluation of arbitrary computations, and is the strongest form. However, it can also be the most inefficient. Partially homomorphic encryption supports only certain computations, but can be more efficient compared to FHE.

At the time of this writing, the overhead of homomorphic encryption is relatively large and using it is relatively rare. However, there are cases where it makes sense, and future advancements may greatly expand where it's applicable.

#### Quantum Computing

One of the large future unknowns in cryptography is the potential impact of general-purpose quantum computers. At the time of this writing, so-called *general-purpose* quantum computers exist, but they are not powerful enough to threaten current cryptographic algorithms. However, research continues to build better quantum computers.

If better *cryptographically relevant* quantum computers are built, they have the potential to break all the historically popular public-key algorithms using an algorithm called *Shor’s algorithm*. It is not known if such quantum computers can be built, and if so, when that will happen. As a result, researchers have developed new public-key algorithms that resist attacks from such quantum computers, an area called *post-quantum cryptography* (PQC) or *quantum-safe cryptography*.

NIST has finalized the first set of PQC standards that resist attacks from such quantum computers. They defined one standard for key establishment, ML-KEM ([FIPS 203](https://csrc.nist.gov/pubs/fips/203/final), based on the Kyber algorithm). NIST also identified several PQC standards for digital signatures:

* ML-DSA ([FIPS 204](https://csrc.nist.gov/pubs/fips/204/final), based on Dilithium)
* SLH-DSA ([FIPS 205](https://csrc.nist.gov/pubs/fips/205/final), based on SPHINCS+)
* FN-DSA ([FIPS 206](https://csrc.nist.gov/pubs/fips/205/final), based on Falcon); at the time of writing this has not been released as a final standard.

Standardization and implementation work continues, so you should check its status whenever you need to use it. The [Open Quantum Safe (OQS)](https://openquantumsafe.org/) project is an open source project that aims to support the transition to quantum-resistant cryptography, by implementing those algorithms and integrating them into protocols and applications. OQS is part of the Linux Foundation’s [Post-Quantum Cryptography Alliance](https://pqca.org/). PQC support is included in [Signal](https://signal.org/blog/spqr/), [Google Chrome (and other products and services)](https://security.googleblog.com/2024/08/post-quantum-cryptography-standards.html), [Apple iMessage (and other products and services)](https://support.apple.com/guide/security/quantum-secure-cryptography-apple-devices-secc7c82e533/web), and [various Cisco products](https://www.cisco.com/site/us/en/about/trust-center/post-quantum-cryptography.html).

In contrast, current symmetric cryptographic algorithms and hash functions are less affected by quantum computers. A quantum computer algorithm called *Grover’s algorithm* speeds up attacks against symmetric ciphers, halving their effective key bit length. That means that 128-bit AES could be broken by a quantum computer (because it would then be equivalent to a 64-bit key today), but 256-bit AES would still be secure (because it would be equivalent to a 128-bit key today). So simply using longer keys and hashes is generally expected to be adequate in a post-quantum world for symmetric cryptographic algorithms and hash functions. The [European Cybersecurity Certification Group Sub-group on Cryptography (ENISA-ECCG) Agreed Cryptographic Mechanisms (2025-04)](https://certification.enisa.europa.eu/document/download/a845662b-aee0-484e-9191-890c4cfa7aaa_en?filename=ECCG%20Agreed%20Cryptographic%20Mechanisms%20version%202.pdf) recommends using symmetric encryption algorithms with key sizes of at least 192 bits, and using hash algorithms that output at least 384 bits.

To ensure comprehensive protection, you should at *least* embrace Cryptographic Agility. This is the design principle of building software so that one cryptographic algorithm can be easily swapped for another (such as moving from RSA to ML-KEM) without requiring major changes to the source code, and enabling easy deployment and upgrade. Also, ensure that key lengths are long enough when using symmetric and hash cryptographic algorithms so that they aren't vulnerable to quantum computers.

Some large organizations are currently recording vast amounts of Internet traffic for later decryption [[European Parliament 2001](https://irp.fas.org/program/process/rapport_echelon_en.pdf)]. This approach is called "harvest now, decrypt later". So if your users are at risk from data capture and decryption years later, you should implement countermeasures *now* against quantum computing.

There are many challenges in implementing PQC. These include:

* Hardware support lag: Cryptographic hardware often dodes not yet support quantum-safe algorithms, delaying adoption.
* Increased ciphertext and signature sizes: Quantum-safe algorithms require many more bytes for ciphertext or digital signatures. That makes them inefficient over transport layers with small Maximum Transmission Units (MTU) such as Zigbee or BLE, where packet fragmentation can degrade performance.

In addition, creating radically new cryptographic algorithms is difficult and risky. About half of all post-quantum cryptography algorithms in NIST's competition have been found to not meet their claimed security levels. One of the leading contenders for post-quantum cryptography was SIKE (Supersingular Isogeny Key Encapsulation), but in 2022 it was discovered that SIKE could be broken by ordinary non-quantum computers [[Goodin 2022](https://arstechnica.com/information-technology/2022/08/sike-once-a-post-quantum-encryption-contender-is-koed-in-nist-smackdown/)]. Thus, if you're adding a post-quantum cryptographic algorithm, we recommend also keeping a pre-quantum layer so that if a break is found in the post-quantum algorithm you are still secure from attacks by traditional computers. This approach of combining post-quantum and transitional cryptographic algorithms is called a Post-Quantum Traditional (PQ/T) hybrid ([IETF 9794](https://datatracker.ietf.org/doc/rfc9794/)). PQ/T hybrid systems add a little more complexity (because you're using two algorithms), but they help counter the significant risk of failure in these newer post-quantum algorithms. [[Bernstein 2024](https://blog.cr.yp.to/20240102-hybrid.html)]
...
* [*The CERT Guide to Coordinated Vulnerability Disclosure*](https://certcc.github.io/CERT-Guide-to-CVD/). In that document the “vendor” is the organization that releases the software and needs to learn about the security vulnerability.
...
### Harden the Environments for Development, Build, Verification (CI/CD pipeline), and Distribution

Most attacks occur when a system is deployed, but increasingly attackers are attacking systems during their development, build, verification, and distribution. These environments include the set of all machines and other infrastructure used to develop the software, including each developer’s systems, version control system(s), build systems, CI/CD pipelines, and so on. The “distribution environment” in particular is the environment used to distribute the resulting built software, e.g., package registries/repositories, container registries/repositories, and so on.

It’s important to secure these environments against unauthorized access or compromise. This includes protecting them against the insertion of malicious code. Assume that attackers are attempting to subvert any system used for software development or distribution, especially any shared systems. Configure these secure environments to harden them against attack, continuously update the tools as necessary for security, and perform regular vulnerability assessments of them. In this section we'll discuss some specifics of how to do that.
...
One simple approach is to have developers use multi-factor authentication (MFA) tokens (aka keys) when accessing external environments. These are hardware devices that prove that the developer possesses the device, and since they are single-purpose they are hard for attackers to subvert. At least use *some* multi-factor mechanism, such as passkeys or passwords combined with some other mechanism. If you use passwords, ensure they are long and not shared between people.

Where practical, perform initial development in a sandboxed environment such as a virtual machine or container. This is especially important if you, like most developers, are using AI/ML agents to aid development. Ensure it's not easily escaped (e.g., don't run a container with `--privileged` or give it access to host's Docker socket `/var/run/docker.sock`). Minimize the unencrypted data in your development environment. Use a password manager if secrets must sometimes be available to your development environment (ideally running in a different environment). Avoid putting private SSH keys in the development environment where you can, but if you must, password protect them.

When using local systems, ensure local storage is encrypted and local firewalls are active. Developers should not run as local administrators for daily tasks. Developers should use increased privileges only when necessary (e.g., sudo and root/admin login).
...
Where practical, harden these environments so they are harder to attack. In many projects, many or all parts of these environments are hosted externally (often in a cloud); make sure the hosting system you choose has adequate security. Once chosen, consult the documentation for the systems you use and configure them to maximize security, e.g., to minimize privileges granted to others. For example, if you use GitHub, look at the [GitHub documentation on securing your repository](https://docs.github.com/en/code-security/getting-started/securing-your-repository). If you have a GitLab installation, [look at the GitLab documentation on securing your installation (“security”)](https://docs.gitlab.com/ee/security/).
...
Your build processes should cache incoming items to increase availability and prevent a loss or degradation of service. Relying solely on the public internet for every build's artifacts can lead to failure or serious degradation; failing to cache build inputs for later use could lead to throttling ([Fox, Brian, "Free Isn't Free: The Hidden Costs of Tooling Decisions in Open Source Infrastructure"](https://www.sonatype.com/blog/free-isnt-free-the-hidden-costs-of-tooling-decisions-in-open-source-infrastructure)).

To further secure the build environment, consider using ephemeral build runners (such as containers or virtual machines) that are created for a single build and destroyed immediately after use. This ensures that if a build is compromised, the attacker cannot remain in the system to subvert future builds. Build runners should have at least restricted network access, at least egress filtering and limits on which systems are accessible. Ideally build processes should be "hermetic" (fully self-contained), so the build can *only* rely on cached incoming items. These measures counter tampering and exfiltration.

Many build environments are defined by code (e.g., Dockerfiles, GitHub Actions YAML, OpenTofu, and Terraform). These configuration files and the blobs they bring in should be scanned for security misconfigurations and known vulnerabilities.This activity is sometimes called Infrastructure as Code (IaC) scanning.
...
Supply chain Levels for Software Artifacts, or SLSA (“salsa”), is a security framework being developed as a checklist of standards and controls to prevent tampering, improve integrity, and secure packages and infrastructure. SLSA is developed by the Open Source Security Foundation (OpenSSF). To learn more, see the SLSA home page at <https://slsa.dev/>.

Another source for good ideas on hardening environments against attack, as well as many other approaches for improving security, are in [Software Supply Chain Best Practices V2](https://github.com/cncf/tag-security/blob/main/community/working-groups/supply-chain-security/supply-chain-security-paper-v2/Software_Supply_Chain_Practices_whitepaper_v2.pdf) from the Cloud Native Computing Foundation (CNCF).
...
Update the tools as practical to harden your environments from attack. Newer tools sometimes add new defenses, so it's usually good to keep them up-to-date. Where practical, use automated tools to make it easy to keep them up-to-date. Sometimes you must use an older version of a tool, for example, to generate an older format (for compatibility with some system) or use a needed capability (that a newer version has lost). If you use an older version, ensure that the older tool doesn't enable attacks to succeed. Often you can do this by providing a protected environment (like a sandbox) for running the older tool. A good long-term strategy is to try to use standards and documented interfaces where practical. These are more likely to be served by newer versions of tools, making it easier to use up-to-date tools.

As we've already discussed, ensure your CI/CD pipeline includes many tools to check the resulting software you're developing. Include linters, SAST, fuzzers, web application scanners (where appropriate), secret scanners, and anything else appropriate. Tools will sometimes report false positives, and will not report all problems, but they provide an important aid in developing secure software.

Similarly, you should perform regular vulnerability assessments of all these environments to prevent exploits. There are various tools for scanning environments for vulnerabilities, and of course AI/ML can be used to examine environments as well. This includes examining the configuration of the environments, examining their run-time behavior, and using tools that simulate attacks. Do these assessments periodically to increase the likelihood of eventually catching a vulnerability in your environment that slipped through everything else.
...
### Continuous Education and Training of Project Members

Software security depends on the people involved. Ensure that all project members receive relevant continuous cybersecurity training, including developers, testers, and maintainers.

Security threats and defenses evolve over time. Training should not only be a "one-time" event such as during onboarding. It must be ongoing to cover new vulnerability types, updated best practices, and changes in the project. It should move from mere "compliance-based" training to a security culture where learning is continuous, expected, and valued. How you implement this training depends on your project. In particular, are all project participants (1) working for the same organization, or (2) members across many organizations?

#### Case 1: Single organization

If all project members work for the same organization (as is common in closed-source and inner-source projects), enforcing and tracking training is generally straightforward:

* Establish a baseline: Require all members to complete foundational security training (such as this course) before they are granted write access to the code repository.
* Integrate into processes: Make security training a part of the standard annual or semi-annual performance review process. Have some sort of long-term progression plan.
* Provide dedicated time: Security training is work. Organizations should provide dedicated time for members to stay current, rather than expecting them to learn on their own time.
* Use internal "security champions": Identify members who are particularly interested in security and provide them with advanced training. They can then act as a resource and "trainer" for the rest of the team, as well as sharing knowledge through internal lunch-and-learns or workshops.

#### Case 2: Multiple Organizations

In many projects, especially open source software (OSS) projects, members may work for different companies or be independent volunteers. In these cases, you cannot "force" training in the same way, but you can incentivize and facilitate it.

* Set clear expectations: Use the project’s CONTRIBUTING.md or GOVERNANCE.md files to explicitly state the security knowledge expected of maintainers and frequent contributors.
* Lower the barrier to entry: Point contributors toward free, high-quality resources. For example, open-source projects can encourage their members to take the OpenSSF’s free training courses.
* Recognition and credentials: Recognize when people have earned a digital certificate for security-related education.
* Use project badging: The OpenSSF Best Practices Badge, baseline program, and SLSA are an excellent way to show that a project cares about security. Make achieving certain levels a goal for the project. This naturally encourages members to learn about them.
* Incorporate security into peer review: Every review of code or documentation is a training opportunity. When a reviewer identifies a security flaw, they should not just fix it; they should explain why it is a vulnerability and point the contributor toward relevant documentation or training. This turns review from a gate into a mentorship opportunity.
* Mentorship: Establish mentorship programs where senior, security-conscious members help guide new contributors.

### Recording changes

Every modification to a system can change its security. Record changes as they occur so you, other developers, and AI systems can review their potential impact, including their impact on security.

The primary mechanism for recording changes should be your version control system (e.g., git). Use descriptive commit messages, focusing on the what and why of the change. If there are important security considerations, note them, so reviewers will know about them. When using git, ensure the subject line (the first line of the commit) provides a concise summary.

Any more detailed security analysis is probably better recorded as a separate document (e.g., in the assurance case). A git commit log can be helpful for understanding how things changed over time, but commit logs tend to be used primarily for determining if a change will be accepted or why a new bug occurred. Record any more detailed analysis as its own document, to make it easier to find and update as needed.

Record changes that impact *users*, including security changes, in the information provided to users (e.g., the CHANGELOG).

Ensure all changes eventually go through your CI/CD pipeline to automatically verify them before distributing your results.
...
* Make sure you *fully* destroy any data you are supposed to destroy.
  * To handle data responsibly at its end-of-life, you must ensure that private data cannot be later recovered by an attacker. Often just deleting or removing a file is not enough; that does not actually remove its contents from most storage devices.
  * To ensure that the data cannot be recovered, consider crypto-shredding (irreversibly destroying the encryption keys), repeatedly overwriting the storage area with random data, or destroying the storage device (e.g., by burning it).

At all times, be *proactive*:

* Engage in *threat hunting*. That is, proactively search for unusual behavior that may indicate the presence of malicious activity that may have slipped past initial defenses. Developers can aid in this by hypothesizing how an attacker might hide their behavior, detecting it, and providing high-fidelity audit information.
* Strive for the *continuous improvement* of security practices. This involves feeding the lessons learned from previous audits, bug reports, and even "near misses" back into your requirements, designs, implementations, and tests. Improve your CI/CD pipeline to prevent similar problems. Update your threat model. Determine if a change in your development process could prevent similar issues.
* Stay vigilant and adapt to the *latest trends* in cybersecurity. Attackers are constantly updating their methods. You must keep up-to-date to counter them. This includes the increased use of AI by attackers and the transition from solely static methods to adding dynamic methods for evaluating user authorization. Proactive thinking is a vital part of effective risk management.
...
The post
[Integrate adversarial attacks in a model training pipeline](https://web.archive.org/web/20210303210800/https://developer.ibm.com/patterns/integrate-adversarial-attacks-model-training-pipeline/),
by Singh et al,
...
* [Software Supply Chain Best Practices V2](https://github.com/cncf/tag-security/blob/main/community/working-groups/supply-chain-security/supply-chain-security-paper-v2/Software_Supply_Chain_Practices_whitepaper_v2.pdf) from the Cloud Native Computing Foundation (CNCF)
...
Baker, Kurt, 2025-03-04, Introduction to Threat Hunting, <https://www.crowdstrike.com/en-us/cybersecurity-101/threat-intelligence/threat-hunting/>
...
CERT, *The CERT Guide to Coordinated Vulnerability Disclosure*, <https://certcc.github.io/CERT-Guide-to-CVD/>
...
Di Paola, Stefano, and Arshan Dabirsiaghi. "Expression Language Injection", 2011-09-12, (<https://web.archive.org/web/20250718054153/https://mindedsecurity.com/wp-content/uploads/2020/10/ExpressionLanguageInjection.pdf>)
...
European Cybersecurity Certification Group Sub-group on Cryptography (ENISA-ECCG) Agreed Cryptographic Mechanisms, 2025-04, <https://certification.enisa.europa.eu/document/download/a845662b-aee0-484e-9191-890c4cfa7aaa_en?filename=ECCG%20Agreed%20Cryptographic%20Mechanisms%20version%202.pdf>
...
Georgiev, Martin; Iyengar, Subodh; Jana, Suman; Anubhai, Rishita; Boneh, Dan; Shmatikov, Vitaly; *The Most Dangerous Code in the World: Validating SSL Certificates in Non-Browser Software*, 2012 <https://www.cs.columbia.edu/~suman/docs/suman_ccs12.pdf>
...
Kaplan-Moss, Jacob, *Not all attackers are equal: understanding and preventing DOS in web applications*, 2020 <https://jacobian.org/2020/sep/11/analyzing-dos-vulnerabilities/>
...
Microsoft,  “3 Ways to Mitigate Risk When Using Private Package Feeds”, <https://web.archive.org/web/20220717015851/https://azure.microsoft.com/en-us/resources/3-ways-to-mitigate-risk-using-private-package-feeds/>
