...
Advances in quantum computing have led some organizations (such as Cloudflare) to recommend that all cryptographic algorithms be quantum-safe by *2029* and that all cryptographic algorithms vulnerable to quantum computers (such as RSA and elliptic curves) be *disabled* by that date to counter downgrade attacks. Once that's done, you'll need to rotate all your secrets ([Cloudflare2026](https://blog.cloudflare.com/post-quantum-roadmap/)).
