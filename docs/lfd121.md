...
If powerful general-purpose quantum computers are built, they have the potential to break all the historically popular public-key algorithms using an algorithm called *Shor’s algorithm*. As a result, researchers have developed new public-key algorithms that resist attacks from such quantum computers, an area called *post-quantum cryptography* (PQC).

NIST has finalized the first set of PQC standards to resist attacks from such quantum computers. These include ML-KEM for key establishment ([FIPS 203](https://csrc.nist.gov/pubs/fips/203/final)), along with ML-DSA ([FIPS 204](https://csrc.nist.gov/pubs/fips/204/final)) and SLH-DSA ([FIPS 205](https://csrc.nist.gov/pubs/fips/205/final)) for digital signatures. This work continues, so you should check its status whenever you need to use it.
