...
We strongly recommend adding AI-powered security analysis to your security verification toolkit.

AI is fundamentally different. Traditional security tools are either static analysis or dynamic analysis tools, but Artificial Intelligence (AI) doesn't fit neatly into either box.  Instead, AI ends up being a powerful way to combine verification approaches.

AI-powered security tools can analyze your software using both static and dynamic approaches, often at the same time. On the static side, an AI can review the output of other static analysis tools, automatically filtering out false positives. AI can also *directly* read through your source code to understand its high-level context, logic, and intent. When AI carefully reviews source code it can find complex vulnerabilities and design flaws that simple rule-based matchers completely miss. On the dynamic side, AI can automatically write custom security test cases, generate smart inputs for your fuzzers, and interact with a running application to observe how it responds to unusual traffic.
...
Some AI models are better than others. However, if you do not have access to the world's best models, or can't afford them, then use what you *can* use. Don't wait for some "perfect tool"; start by using what you have available.
