...
There are many ways to authentication that a user is who they say they are. The most common factors (evidence) are:
...
* *Protection against phishing*: An attacker can't create a fake login website, trick the user into logging in, and later log in to the real real site. The signature is bound to the website's origin, and the server provides a unique 'challenge' for every request, ensuring that intercepted login data cannot be reused.
...
* *Invulnerable to password attacks*: Some users reuse passwords; attackers who discovers that reused password can use it to attack other sites. In addition, passwords are vulnerable to brute force attacks. Private keys that are specific to a particular site don't have these weaknesses. [Web Authentication API](https://developer.mozilla.org/en-US/docs/Web/API/Web_Authentication_API)
...
While OAuth is powerful, it must be implemented correctly. In particular, security best practices today require using Proof Key for Code Exchange (**PKCE**) in OAuth to prevent authorization code injection attacks. PKCE checks to ensure that the entity requesting the authorization code (used to get the access token) is the exact same entity exchanging it.
