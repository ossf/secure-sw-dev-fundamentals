...
One of the simplest ways to ensure an attacker cannot trigger vulnerabilities from integer overflows and underflows is to do input validation on all untrusted numbers. In particular, enforce minimum and maximum values on all untrusted integers. If the accepted inputs cannot lead to a calculation that would trigger an integer overflow or underflow, then there are no such vulnerabilities.
