...
כדי *לקחת* את הקורס, אנא עבור אל [דף האינטרנט OpenSSF על קורסים יסודות פיתוח תוכנה מאובטחת](https://openssf.org/training/courses/).
...
1. פיתוח תוכנה מאובטחת - חלק א', דרישות, תכנון המערכת ושימוש חוזר [מכסה את היסודות, הדרישות, העיצוב, השימוש החוזר] -  ~ קורס של 2-4 שעות בלבד, ~ 4-5 שעות בסך הכל כולל בדיקות וחקירת משאבים / קישורים שסופקו
...
קורס זה מתמקד בסוגיות יישום מרכזיות: אימות קלט (כגון מדוע יש להשתמש ברשימות מותרות ולא בדניליסטים), עיבוד נתונים בצורה מאובטחת, קריאה לתוכניות אחרות, שליחת פלט וטיפול בשגיאות. הוא מתמקד בצעדים מעשיים שאתה (כמפתח) יכול לנקוט כדי להתמודד עם סוגי ההתקפות הנפוצים ביותר.
...
<ul>
<li>אימות אבטחה: כיצד לבחון תוכנות, לכלול כמה סוגי כלים מרכזיים וכיצד ליישם אותם באינטגרציה רציפה (CI). זה כולל למידה על סורקי קוד אבטחה/כלי בדיקת אבטחת יישומים סטטיים (SAST), ניתוח הרכב תוכנה (SCA)/כלי ניתוח תלות, תנורים וסורקי יישומי אינטרנט.</li>
...
* שחרור מידע פרטי/סודי (המכונה *גם "לאבד סודיות"*)
...
1. **בחנים**. אנו שואלים שאלות חידון לאורך הדרך כדי לעזור לחזק מושגים. קל להתנתק מהספרים והסרטונים המסורתיים, מתוך מחשבה שאתה מבין את מושגי הליבה גם כשאתה לא. לעומת זאת, החידונים עוזרים לחזק את מושגי הליבה כך שתבינו אותם.
...
* **זמינות**<br>"ממשיך לעבוד בנוכחות התקפה." - התוכנה ממשיכה לעבוד בזמן שהיא תחת מתקפה. התקפת מניעת שירות (DoS) היא התקפה שמנסה להפוך את התוכנה ללא זמינה עוד.
...
* **זיהוי ואימות (I&A)**<br>דורשים מהמשתמשים להזדהות ולהוכיח (לאמת) את זהותם לפני שהם עושים כל דבר הדורש הרשאה. לדוגמה, הם עשויים להשתמש בשם משתמש או בכתובת דואר אלקטרוני כזהותם, ולהשתמש בסיסמה או באסימון חומרה כדי לאמת שהם באמת משתמשים אלה. זה נעשה בדרך כלל על ידי תהליך התחברות.
...
* **ביקורת** (aka logging)<br>תעד אירועים חשובים כדי לסייע בזיהוי התקפות ובהתאוששות מהן. בדרך כלל אירועים אלה כוללים כניסה, התנתקות ושינוי מידע חשוב. ביקורת היא לעתים קרובות קריטית ליישום דרישות אי-התכחשות / אחריותיות.
...
דרישות אפילו לא *צריך* להיות כתוב כדי לשמש, במיוחד עבור פרויקט קטן. עם זאת, לפחות במקרה של אבטחה, כדאי לרשום את דרישות האבטחה ברמה גבוהה במקום אחד. לאחר מכן, כאשר מישהו חושב להשתמש בתוכנה שלך או לשנות אותה, יהיה לו מושג מה המערכת מנסה להשיג לצורך אבטחה.
...
1. **סודיות** ("אין קריאה לא מורשית")<br>זהה מידע שאין לחשוף בפומבי, כגון מידע פרטי על אנשים ומערכות. למי צריך להיות מותר לראות את זה? האם אתה יכול להימנע מלקבל את המידע הזה בכלל (שכן אתה לא יכול לחשוף את מה שאין לך)? אם אתה מאחסן פרטי סיסמה כדי שאנשים יוכלו להיכנס למערכת שלך (המכונה אימות "נכנס"), עליך לאחסן פרטי סיסמה אלה באמצעות אלגוריתמים מיוחדים המיועדים לכך (כגון Argon2id), כפי שנדון בהמשך.

2. **יושרה** ("אין שינוי לא מורשה")<br>זהה מידע שרק אנשים מסוימים צריכים להיות מורשים לשנות, ומי זה.
...
7. **ביקורת/רישום איזה**<br>מידע/אירועים עליך לתעד? בדרך כלל אתה לפחות מתעד כניסה, התנתקות ואירועים חשובים כמו יצירה ומחיקה של חשבון משתמש. בדרך כלל מערכת צריכה לרשום מתי משהו קרה (תאריך ושעה), מה קרה, איזה רכיב מערכת עשה את זה ומי גרם לזה לקרות.
...
האיגוד הבינלאומי ללא מטרות רווח [של מומחי פרטיות (IAPP) מגדיר פרטיות](https://iapp.org/about/what-is-privacy/) כ*"הזכות להשתחרר, או חופש מהתערבות או חדירה".* באופן ספציפי יותר, הוא אומר *"פרטיות מידע היא הזכות לקבל שליטה מסוימת על האופן שבו המידע האישי שלך נאסף ונעשה בו שימוש... לתרבויות שונות יש דעות שונות באופן נרחב לגבי מהן זכויותיו של אדם בכל הנוגע לפרטיות וכיצד יש להסדיר אותן". * הם גם מנוגדים לפרטיות ולאבטחה: *"פרטיות נתונים מתמקדת בשימוש ובפיקוח על נתונים אישיים - דברים כמו קביעת מדיניות כדי להבטיח שהמידע האישי של הצרכנים נאסף, משותף ונעשה בו שימוש בדרכים מתאימות".*

במילים אחרות, פרטיות עוסקת בהגנה על נתונים אישיים על אנשים מפני שימוש לרעה.

#### מדוע פרטיות חשובה?

בעוד שחלקם טענו כי פרטיות כבר אינה אפשרית או רלוונטית, רבים אחרים חולקים על כך, וחוקים רבים נקבעו ברחבי העולם כדי להגן על הפרטיות. אחד התקצירים הנגישים לעמדה הרווחת שלפיה פרטיות חשובה הוא הרצאת TED של גלן גרינוולד  "[*מדוע הפרטיות חשובה"*](https://www.ted.com/talks/glenn_greenwald_why_privacy_matters) (2014). הנה כמה מהנקודות שלו (ראו את השיחה לפרטים):

* אנשים שאומרים*, "אם אתה עושה משהו שאתה לא רוצה שאנשים אחרים ידעו, אולי אתה לא צריך לעשות את זה מלכתחילה"* עוסקים בהוצאה עצמית קיצונית; * "מה שהם בעצם אומרים זה 'הסכמתי להפוך את עצמי לאדם כל כך לא מזיק ולא מזיק ולא מעניין, שאני בעצם לא חושש שהממשלה תדע מה אני עושה'".*

* רבים מאותם אנשים [שטוענים את הטענות האלה] אינם פועלים למעשה בדרך זו, למשל, הם ינקטו צעדים רבים כדי להשיג פרטיות לעצמם.

* *"יש סיבה שכל כך משתוקקים לפרטיות באופן אוניברסלי ואינסטינקטיבי... כאשר אנו נמצאים במצב שבו ניתן לפקח עלינו, שבו ניתן לצפות בנו, ההתנהגות שלנו משתנה באופן דרמטי... יש עשרות מחקרים פסיכולוגיים שמוכיחים שכשמישהו יודע שאפשר לצפות בו, ההתנהגות שהוא עוסק בה היא הרבה יותר קונפורמיסטית ותואמת".*
...
אם לא תצליחו בכך, צמצמו את המידע האישי למה שאתם בהחלט דורשים. אם אתה חייב לאסוף מידע על אנשים, עליך לספק להם מגוון הגנות, לכל הפחות אלה הנדרשות על פי החוק והרגולציה. זה יכול להיות מסובך, כי חוקים ותקנות רבים עשויים לחול.
...
בארצות הברית (ארה"ב) אין חוק פרטיות מידע מקיף בכללותו. במקום זאת, לממשל הפדרלי בארה"ב יש מספר חוקים המכסים נסיבות ספציפיות. זה כולל את חוק הזכויות החינוכיות המשפחתיות והפרטיות של 1974 (FERPA) עבור רשומות חינוך של תלמידים, חוק הניידות והאחריות של ביטוח בריאות משנת 1996 (HIPAA) עבור נתונים הקשורים לבריאות, חוק הגנת הפרטיות המקוונת של ילדים משנת 1998 (COPPA) עבור נתונים הקשורים לילדים, וחוק עסקאות אשראי הוגנות ומדויקות משנת 2003 (FACTA) עבור נתונים פיננסיים מסוימים.

 [חוק הפרטיות של ארה"ב משנת 1974 (5 U.S.C. 552a)](https://www.govinfo.gov/content/pkg/USCODE-2018-title5/pdf/USCODE-2018-title5-partI-chap5-subchapII-sec552a.pdf) מחייב כיצד סוכנויות פדרליות בארה"ב חייבות לשמור רשומות על אנשים שהם אזרחי ארה"ב וזרים תושבי קבע חוקיים. לדוגמה, הם חייבים:
...
כדי לעבד נתונים אישיים, הם חייבים להיות חוקיים, כלומר עליהם ליפול לפחות לאחת מכמה קטגוריות, כולל בין היתר את הקטגוריות הבאות:
...
"מדיניות איסוף ושימוש בנתוני טלמטריה"[* של קרן לינוקס *](https://www.linuxfoundation.org/telemetry-data-policy/) מציגה דיון קצר בכמה מהנושאים שיש לקחת בחשבון לפני יישום איסוף נתוני טלמטריה, כמו גם דיון בגישת הקרן לניהול השימוש בטלמטריה על ידי קהילות פרויקט הקוד הפתוח שלה. זה עשוי להיות שימושי עבורך בהקשרים אחרים.
...
כל החיים כרוכים בסיכון. זה לא מציאותי לצפות שלא יהיו סיכונים בחיים. בפרט, ישנם סיכונים לכל מי שמשתמש בתוכנה שאתה מפתח מכיוון שהיא עלולה להיות בעלת פגיעויות. כאשר אתה מפתח תוכנה, סביר להניח שתעשה טעויות, וחלק מהטעויות הללו עלולות להוביל בסופו של דבר לפגיעויות אבטחה. מישהו עשוי אפילו לנסות להכניס בכוונה פגיעויות או קוד זדוני לתוכנה שלך, או לתוכנה שאתה תלוי בה, במהלך הפיתוח שלה. אפילו טכניקות חזקות מאוד להתמודדות עם פגיעויות חייבות להתבסס על הנחות או יכולות רק לחסל *כמה* סיכונים  הקשורים לאבטחה, כך ששוב, זה לא מציאותי לצפות שלא יהיו סיכונים.
...
במאמרו "[*תהליך הביטחון" *](https://www.schneier.com/essays/archives/2000/04/the_process_of_secur.html) (2000), ברוס שנייר הסביר באופן מפורסם כי
...
אל תשווה בין רשימות, הנחיות ועצות לבין אבטחה. לעתים קרובות הם *שימושיים*, מכיוון שהם יכולים לעזור לך לזהות סיכונים ודרכים סבירות לטפל בהם. רשימות פעולות לביצוע, הנחיות וטיפים טובים יכולים לחסוך לך הרבה זמן וצרות, והם גם עזרים נהדרים לעזור לאחרים להעריך את האבטחה של תוכנות מסוימות. טובים בנויים על ניסיון של אחרים, ואתה תהיה טיפשי להתעלם מהחוויה הזו.
...
\>\>מטרת פיתוח תוכנה מאובטחת היא לחסל את כל סיכוני האבטחה האפשריים. אמת או שקר?<<
...
* אספקה רציפה (CD או CDE) נועדה להבטיח ש*"יישום נמצא תמיד במצב מוכן לייצור לאחר שעבר בהצלחה בדיקות אוטומטיות ובדיקות איכות [על ידי שימוש בפרקטיקות] כדי לספק תוכנה באופן אוטומטי לסביבה דמוית ייצור"* (Mojtaba Shahin, מוחמד עלי באבר ו- Liming Zhu[*, אינטגרציה רציפה, אספקה ופריסה: סקירה שיטתית על גישות, כלים, אתגרים ושיטות עבודה*](https://arxiv.org/abs/1703.07019) , 2017). שים לב שהתוכנה לא בהכרח משוחררת או נפרסת ללא שלב אישור נפרד.
...
* DevOps מתמקדת בתיאום ובשיתוף פעולה בין צוותי פיתוח התוכנה (Dev) ותפעול ה-IT (Ops) (Mike Loukides, [*Revisiting "What Is DevOps",*](http://radar.oreilly.com/2014/06/revisiting-what-is-devops.html) 2014), למשל, כדי לקצר את זמן הפיתוח והפריסה. בפועל, זה כולל בדרך כלל אספקה רציפה (CDE) ועשוי לכלול פריסה רציפה (CD).

* DevSecOps (נקרא גם SecDevOps) הוא DevOps, אך באופן ספציפי משלב חששות אבטחה בתהליך הפיתוח והתפעול (Red Hat, [*מה זה DevSecOps? *](https://www.redhat.com/en/topics/devops/what-is-devsecops))
...
כל אלה תלויים בבדיקות אוטומטיות ובבדיקות איכות, ומבחינת אבטחה, מה שקריטי הוא שיש לשלב כלים לבדיקת פגיעויות אבטחה ובעיות אבטחה פוטנציאליות בבדיקות האוטומטיות ובבדיקות האיכות הללו. לדוגמה, עליך לוודא שכלים נמצאים בצינור ה- CI שלך שבודקים אם קיימות בעיות אבטחה שונות, כך שבעיות אבטחה יזוהו מוקדם. כלי אבטחה שלוקח זמן רב להפעיל עשויים להיות מופעלים במקביל אך לשמש כ"שער "עבור CDE. בהמשך הקורס נדון בהרחבה על כלים לתמיכה באבטחה.
...
2. **הגנה**<br>*"לפתח וליישם אמצעי הגנה מתאימים כדי **להבטיח** אספקה של שירותים קריטיים".*
...
5. **שחזור**<br>*"לפתח וליישם פעילויות מתאימות לשמירה על תוכניות עמידות ולשיקום יכולות או שירותים שנפגעו עקב תקרית אבטחת סייבר".*
...
פגיעות היא פשוט אי עמידה בדרישת אבטחה כלשהי. בדרך כלל פגיעויות אינן מכוונות, אך פגיעויות יכולות להיות מכוונות. לדוגמה, ייתכן שמישהו הכניס במכוון קוד זדוני (או לפחות ניסה לעשות זאת) בתוכנה שבה אתה עושה שימוש חוזר או מפתח, כגון דלת אחורית (דרך להשיג גישה לא מורשית) או פצצת לוגיקה (קוד המבצע פונקציה זדונית כאשר מתקיימים התנאים שצוינו). קורס זה מתמקד אך ורק בתוכנה, אם כי כמובן שגם לחומרה יכולות להיות נקודות תורפה.

החברה המודרנית תלויה בתוכנה (ובחומרה), וכתוצאה מכך, חל גידול מסיבי במספר הפגיעויות הידועות בציבור. זה הקשה על מענה על שאלות פשוטות כמו  "*האם תיקנת את הפגיעות המסוימת הזו"*? לאחר מכן, נתאר כמה מאמצים שנעשו כדי לזהות ולטפל פגיעויות ידועות.

#### דיווח וטיפול בפגיעויות - סיכום קצר

ישנם אנשים רבים שמצאו, מסיבה זו או אחרת, פגיעויות אבטחה בתוכנה. יש אנשים, שנקראים חוקרי אבטחה, שהופכים את מציאת נקודות התורפה לחלק מהקריירה שלהם.
...
ניתן לקבץ את הרוב המכריע של הפגיעויות לקטגוריות. זה מתברר להיות מאוד שימושי; ברגע שאנחנו מזהים קטגוריות, אנחנו יכולים לקבוע אילו מהן נפוצות ואילו צעדים אנחנו יכולים לנקוט כדי למנוע מפגיעות מסוג זה להתרחש שוב.
...
2. [**רשימת 25 המובילים של CWE**](https://cwe.mitre.org/top25/archive/2019/2019_cwe_top25.html)<br>זוהי רשימה של הסוגים הנפוצים והקריטיים ביותר של פגיעויות. הוא נוצר על ידי צוות ספירת החולשות הנפוצות (CWE) על ידי ניתוח נתונים על פגיעויות ידועות לציבור לאורך שנים רבות. רשימה זו יכולה להיות מיושמת על כל תוכנה, אך נפוץ במיוחד להחיל אותה על תוכנה שאינה יישום אינטרנט (שכן רשימת OWASP מתמקדת ביישומי אינטרנט). מוזרות מעניינת אחת: הם מזהים חולשות חשובות מעבר ל-25 הראשונות, כך שניתן לראות מספרים גדולים מ-25 המשויכים לרשימה זו.
...
הרוב המכריע של התוכנות שבהן אתה עושה שימוש חוזר יהיו בדרך כלל תוכנות קוד פתוח (OSS). אז בואו נתמקד בטיפים כיצד להעריך את OSS לפני השימוש החוזר בו. חלק מהטיפים הללו יחולו גם על תוכנות קוד סגור.
...
3. **האם זה מתוחזק?**  תוכנה לא מתוחזקת היא סיכון; רוב התוכנות זקוקות לתחזוקה שוטפת. אם היא לא מאובטחת, סביר להניח שהיא גם לא בטוחה.
    ...
    4. האם יש מהדורות אחרונות או הכרזות מהמתחזק(ים) שלו?
...
7. **האם יש לו שימוש משמעותי?**  תוכנה עם משתמשים רבים או משתמשים גדולים עשויה להיות בלתי הולמת לשימושך. עם זאת, סביר יותר להניח שתוכנה הנמצאת בשימוש נרחב תציע מידע שימושי על אופן השימוש המאובטח בה, ויותר אנשים ידאגו לאבטחה שלה. בדוק אם שם דומה הוא פופולרי יותר - זה יכול להצביע על התקפת הקלדה.
...
9. **מהי ההערכה שלך לגבי הקוד שלה?**  אפילו סקירה קצרה של קוד המקור של התוכנה, והשינויים שלה לאורך זמן, יכולה לתת לך כמה תובנות. להלן דברים שיש לקחת בחשבון:
    ...
    4. האם יש ראיות לכך שהתוכנה זדונית? לפי [_אוסף הסכינים של Backstabber_](https://arxiv.org/abs/2005.09535), בדוק את סקריפטי ההתקנה / שגרות זדוניות, בדוק אם יש סינון נתונים מ~/.**ssh** ומשתני סביבה, וחפש ערכים מקודדים / מעורפלים המבוצעים. בדוק את פעולות ה-Commit האחרונות לאיתור קוד חשוד (ייתכן שתוקף הוסיף אותן לאחרונה).
...
> ב-2019-12-01 גילה מפתח התוכנה הגרמני לוקאס מרטיני כי שתי ספריות Python במאגר PyPI הפופולרי (Python Package Index) יישמו התקפות הקלדה. חבילות זדוניות אלה היו גונבות מפתחות פרטיים SSH ו- GPG ממפתחים שהשתמשו בהם. החבילה  הזדונית `jeIlyfish` חיקתה את החבילה הלא זדונית `jellyfish` ועשתה את הנזק (שים לב שבשם החבילה הזדונית התו השלישי הוא אותיות רישיות "`I`", ולא אותיות קטנות "`l`"). אותו תוקף העלה גם חבילה זדונית בשם `python3-dateutil` אשר חיקתה את הספרייה הפופולרית `dateutil` עבור Python3. החבילה  הזדונית `python3-dateutil` לא כללה קוד זדוני בעצמה, אלא במקום זאת טענה את החבילה הזדונית `jeIlyfish` כתלות. החבילה  הזדונית הייתה `python3-dateutil` ב-PyPI רק יומיים, אבל החבילה הזדונית הייתה `jeIlyfish` זמינה במשך כמעט שנה. שתי הספריות הוסרו על ידי PyPI ביום שבו PyPI קיבל הודעה ("[שתי ספריות Python זדוניות נתפסו גונבות מפתחות SSH ו- GPG" על ידי Catalin Cimpanu, ZDNet, 2019](https://www.zdnet.com/article/two-malicious-python-libraries-removed-from-pypi/)).
...
1. ודא שיש לך בדיוק את השם הנכון. התקפה נפוצה נקראת "שגיאת הקלדה". בשגיאות הקלדה, תוקף ייצור שם חבילה או שם תחום הדומים באופן מכוון וזדוני לרכיב תוכנה הנמצא בשימוש נרחב, וישתמש בשם מטעה זה כדי להפיץ גירסה זדונית של אותה תוכנה. [Ohm & all, 2020](https://arxiv.org/abs/2005.09535) מצא כי *"רוב החבילות הזדוניות [OSS] מחקות את שמות החבילות הקיימות באמצעות שגיאות הקלדה".* לדוגמה:
    ...
    3. בדוק את תאריך ההפצה של החבילה. המבוגר יותר הוא לעתים קרובות זה שרצית.
...
2. הקפד להוריד ולהתקין את התוכנה בצורה אמינה:
    ...
    3. שקול להוריד את התוכנה, אך לאחר מכן התקן אותה והשתמש בה רק כמה ימים לאחר מכן לאחר אימות ששום דבר לא השתנה. בדרך זו, אם אתר ההפצה הוא חתרני באופן זמני בעת הורדת התוכנה, אבל הוא תוקן במהירות, אתה לא תשתמש בגירסה חתרנית. זה לא תמיד מעשי, שכן אתה עלול להיות יותר מדי ממהר לחכות, אבל במקרים מסוימים זה קל לעשות.

    4. נסה להימנע משימוש בצינור למעטפת (כגון  **תלתל ... | sh**) כדי להוריד ולהתקין תוכנה. ברור שאינך יכול להוריד ולעכב את ההתקנה כאשר אתה משתמש בצינור למעטפת. בנוסף, תוקפים החותרים תחת אתר מקור יכולים לזהות בקשה מצינור למעטפת ולחתור באופן סלקטיבי תחת משתמשי pipe-to-shell, אשר מעצם הגדרתם אינם בודקים את מה שהם מורידים (ראה [ספרו של פיל "זיהוי השימוש ב-curl | bash" צד שרת"](https://www.idontplaydarts.com/2016/04/detecting-curl-pipe-bash-server-side/)). השימוש בצינור למעטפת הופך את החתרנות באתר המקור לקשה הרבה יותר לזיהוי ולנטרול. זה גם מקשה על הבנת הגרסה בפועל שהורדת והתקנת באופן סמכותי - כך שאיבדת למעשה שליטה מסוימת בגרסה, ואינך יכול לסמוך על אחרים כדי שתוכל לקבוע מה קרה. כן, התוכנית המותקנת יכולה לדווח על גירסה, אך תוכניות יכולות לדווח על כל מספר ויכולות לדווח על אותו מספר עבור גירסאות בפועל שונות. בקיצור, הסיכונים שלך גדלים אם אתה משתמש צינור למעטפת.
...
4. השתמש בתכונות אימות בצד הלקוח. גישה אחת היא לאכוף הצמדת תלות המכונה הצמדת גרסה, כלומר, לדרוש שימוש בגרסה ידועה ספציפית. ניתן לאכוף זאת על ידי דרישת ערך גיבוב קריפטוגרפי ספציפי (המכונה "טביעת אצבע דיגיטלית") של חבילה. גישה נוספת היא אימות תקינות כדי לוודא שחבילה שהורדת זהה לפעם הראשונה שהורדה.
...
בפועל, יהיו לך רכיבי תוכנה רבים שנעשה בהם שימוש חוזר, והם יצטרכו להתעדכן מדי פעם. לפעמים תימצא פגיעות באחת, ובמקרה זה עליך לקבל הודעה במהירות ולהיות מוכן לעדכן במהירות. כתוצאה מכך, עליך לנהל רכיבים שנעשה בהם שימוש חוזר:
...
2. הסתמך רק על ** ממשקים והתנהגות מתועדים, והימנע מממשקים מיושנים, כדי למקסם את הסבירות ליכולת לעדכן תוכנות שנעשה בהן שימוש חוזר בעת הצורך.
